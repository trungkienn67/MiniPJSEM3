...
function lib_countDown(selector) {
	...
	// Ki?u th?i gian ??t tag endtime_4/15/2018 15:10:00
	...
	var x = setInterval(function () {
		...
		selector.html("<div><strong>" + days + "</strong><span>Ngày</span></div>" + "<div><strong>" + hours + "</strong><span>Gi?</span></div>" + "<div><strong>" + minutes + "</strong><span>Phút</span></div>" + "<div><strong>" + seconds + "</strong><span>Giây</span></div>");
...
$(document).ready(function () {
	$("#nav-mobile").mmenu({
		...
		navbars: [{
			...
			type: 'tabs',
			content: [
				'<a href="#panel-menu"><i class="fa fa-bars"></i> <span>Danh m?c</span></a>',
				'<a href="#panel-account"><i class="fa fa-user"></i> <span>Tài kho?n</span></a>'
			]
		}, {
			content: ['prev', 'close']
		}]
...
function action(current, datasection) {
	...
	$('.' + datasection + ' ul.tabs.tabs-title.hidden-xs li').each(function (e) {
		if ($(this).attr('data-tab') == 'tab-' + current) {
			...
			//N?u ?ã load r?i thì không load n?a
